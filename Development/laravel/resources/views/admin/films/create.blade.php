...
<div class="min-h-screen bg-gradient-to-b from-black via-[#141414] to-[#1a1a1a] text-white px-8 py-16 relative overflow-hidden">
    ...
    <form action="{{ route('admin.films.store') }}" method="POST" enctype="multipart/form-data"
      ...
      <div>
        ...
        <input type="text" name="judul"
               class="block w-full border border-red-800/60 rounded-lg bg-[#0d0d0d] text-gray-200 p-3" required>
      </div>
      ...
      <div>
        ...
        <textarea name="sinopsis" rows="4"
                  class="block w-full border border-red-800/60 rounded-lg bg-[#0d0d0d] text-gray-200 p-3" required></textarea>
      </div>
      ...
      <div class="grid grid-cols-2 gap-6">
        <div>
          ...
          <input type="number" name="tahun_rilis" min="1900" max="2100"
                 class="block w-full border border-red-800/60 rounded-lg bg-[#0d0d0d] text-gray-200 p-3" required>
        </div>
        <div>
      ...
      <div class="pt-10 text-center">
        <button class="bg-red-700 hover:bg-red-800 px-10 py-3 rounded-xl font-bold text-white shadow-[0_0_30px_rgba(220,38,38,0.6)] hover:scale-105 transition-all duration-300">
          💾 Simpan Film
        </button>
      </div>
    </form>
  ...
  {{-- Modal Tambah Genre --}}
  <div id="genre-modal" class="fixed inset-0 bg-black/90 backdrop-blur-sm flex justify-center items-center hidden z-50">
    <div class="bg-[#0d0d0d] p-8 rounded-2xl border border-red-700/60 w-96">
      ...
      <form id="genre-form" class="space-y-5">
        ...
        <div>
          ...
          <input type="text" id="genre-name"
                 class="w-full p-3 bg-[#101010] border border-red-800/60 rounded-lg text-gray-200" required>
        </div>
        <div class="flex justify-end space-x-4">
          <button type="button" onclick="closeGenreModal()"
                  class="px-5 py-2 bg-gray-700 hover:bg-gray-800 rounded-lg text-white">Batal</button>
          <button type="submit"
                  class="px-5 py-2 bg-red-700 hover:bg-red-800 rounded-lg font-semibold text-white">Simpan</button>
        </div>
      </form>
    </div>
  </div>
